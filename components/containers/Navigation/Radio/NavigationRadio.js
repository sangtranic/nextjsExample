            ...
            <nav className="bg-red-800">
                        ...
                        <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                            ...
                            <div className="hidden sm:ml-6 sm:block">
                                <div className="flex space-x-4">
                                    <RadioLink to={`/radio.html`} className="bg-red-900 text-white rounded-md px-3 py-2 text-sm font-medium" aria-current="page">
                                        Trang chủ
                                    </RadioLink>
                ...
                <div className={`sm:hidden ${openMenu === true ? 'block' : 'hidden'}`} id="mobile-menu">
                    <div className="space-y-1 px-2 pb-3 pt-2">
                        <RadioLink to={`/radio.html`} className="bg-red-900 text-white block rounded-md px-3 py-2 text-base font-medium" aria-current="page">
                            Trang chủ
                        </RadioLink>

                        {
                            menu.map((val, index) => (
                                <RadioLink key={val?.uniqId} to={val?.slug &&
                                    val?.uniqId && `/podcast/${val.slug}.${val.uniqId}.html`}
                                    className="text-gray-300 hover:bg-red-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium">
                                    {val.name}
                                </RadioLink>
                            ))
                        }
